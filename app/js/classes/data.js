...
var data =
{
			...
			if (object.output != undefined) {
				for (var i = 0; i < object.output.length; i++) {
					if (object.output[i].type == 0) {
						node.text.push({'id': ko.observable(object.output[i].content)});
					}
					if (object.output[i].type == 2) {
						node.text.push({'id': ko.observable(object.output[i].content.text)});
	...
	filterNodeData: function(node)
	{
		...
		for (let i = 0; i < node.text().length; i++) {
			text.push(node.text()[i].id());
		}

		for (let i = 0; i <text.length; i++) {
			output.push({"type":"0", "content": text[i]});
		}
	...
	getSaveData: function(type)
	{
		...
		else if (type == FILETYPE.XML)
		{
			...
			for (i = 0; i < content.length; i ++)
			{
				...
				output += "\t\t<text>" + content[i].text + "</text>\n";
